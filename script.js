...
function commentCreator() {
  ...
  comment =
    comment.length < row.length
      ? `//${indents}# ${inputText.value}${indents} //`
      : comment;
...
${row}`;
  ...
  if (inputText.value.length % 2 === 0) {
    template2 = `// ${"-".repeat(
      (inputSize.value - inputText.value.length - 2) / 2
    )} ${inputText.value} ${"-".repeat(
      (inputSize.value - inputText.value.length - 1) / 2
    )} //`;
  } else {
    template2 = `// ${"-".repeat(
      (inputSize.value - inputText.value.length - 1) / 2
